...
public class CalmingCandleBlockEntity extends BlockEntity {
    private static final BooleanProperty LIT = BooleanProperty.of("lit");
    private static final int RADIUS = 20;
    public CalmingCandleBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CALMING_CANDLE, pos, state);
    }
    ...
    public void tick(World world, BlockPos pos, BlockState state) {
        if (state.get(LIT)) {
            ...
            for (LivingEntity entity : livingEntities) {
                if (!entity.hasStatusEffect(ModEffects.CALM)) {
                    entity.addStatusEffect(new StatusEffectInstance(ModEffects.CALM, 200, 0, true, false, true));
                }
            }
        }
    }
